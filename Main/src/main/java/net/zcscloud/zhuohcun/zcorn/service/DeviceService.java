...
public class DeviceService{
    ...
    public int ismatch(String devid,String token) throws JSONException, IOException {
        if(userService.verifyToken(token)=="0"){
            ...
            if(c.get("userid").equals(deviceProxy.getusidbydevid(Integer.parseInt(devid)))){
                return 1;
            }else {
                return -1;
            }
    ...
    @Scheduled(fixedRate = 5000) // this will be performed every 5 seconds, to turn on all necessary operators    <==> while true
    ...
    @Scheduled(fixedRate = 5000) // this will be performed every 5 seconds, to turn off all operators when recovering    <==> while true
    ...
    public PredictionResponse predicthon(MultipartFile file, int topK) throws IOException {
    ...
    public PredictionResponse predictwd(MultipartFile file, int topK) throws IOException {
        okhttp3.RequestBody body = new MultipartBody.Builder()
                .setType(MultipartBody.FORM)
                .addFormDataPart("file", file.getOriginalFilename(),
                        okhttp3.RequestBody.create(file.getBytes(), MediaType.parse(file.getContentType())))
